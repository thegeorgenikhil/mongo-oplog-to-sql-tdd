...
func generateUpdateSQL(oplogObj OplogEntry) (string, error) {
	...
	case "u":
		...
		for columnName, value := range oplogObj.O2 {
			whereClauseCols = append(whereClauseCols, fmt.Sprintf("%s = %s", columnName, getColumnValue(value)))
		}

		sql = fmt.Sprintf("%s WHERE %s;", sql, strings.Join(whereClauseCols, ", "))
...
func generateDeleteSQL(oplogObj OplogEntry) (string, error) {
	...
	case "d":
		...
		sort.Strings(whereColumnValues)
